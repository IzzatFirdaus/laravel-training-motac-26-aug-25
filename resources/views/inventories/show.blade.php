...
<main id="main-content" class="myds-container py-4" role="main" tabindex="-1" aria-labelledby="page-heading">
  ...
  <nav aria-label="Breadcrumb" class="mb-4">
    <ol class="d-flex list-unstyled myds-text--muted myds-body-sm m-0 p-0">
      ...
      <li class="mx-2" aria-hidden="true">/</li>
      <li aria-current="page" class="myds-text--muted">{{ $inventory->reference ?? ('#'.$inventory->id) }}</li>
    </ol>
  </nav>

  <section class="myds-grid myds-grid-desktop myds-grid-tablet myds-grid-mobile">
    <div class="desktop:col-span-8 tablet:col-span-8 mobile:col-span-4">
      <header class="mb-3">
        <h1 id="page-heading" class="myds-heading-md font-heading font-semibold">Inventori #{{ $inventory->id }}</h1>
        <p class="myds-body-sm myds-text--muted mb-0">Butiran penuh item inventori.</p>
      </header>
      ...
      <div class="bg-surface border rounded p-4 shadow-sm">
        ...
        <div class="mt-4 d-flex gap-2">
          ...
          @can('update', $inventory)
            <a href="{{ route('inventories.edit', $inventory->id) }}" class="myds-btn myds-btn--primary">Edit</a>
          @endcan
        ...
        @if(isset($inventory->vehicles) && $inventory->vehicles->count())
          ...
          <ul class="myds-list myds-list--bare">
            @foreach($inventory->vehicles as $v)
              <li><a href="{{ route('vehicles.show', $v->id) }}" class="text-decoration-none">{{ $v->name ?? '—' }}</a> <span class="myds-text--muted">(ID: {{ $v->id }})</span></li>
            @endforeach
          </ul>
        @endif
      </div>
    ...
    <aside class="desktop:col-span-4 tablet:col-span-8 mobile:col-span-4">
      <div class="bg-muted p-4 rounded">
        ...
        <p class="small myds-text--muted mb-0">Tarikh dicipta: {{ $inventory->created_at?->format('Y-m-d H:i') ?? '—' }}</p>
      </div>
    </aside>
  </section>
</main>
