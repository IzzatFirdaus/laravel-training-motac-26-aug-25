...
@section('content')
{{-- MYDS Page Header following citizen-centric design --}}
<header class="mb-4">
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-start gap-3">
        <div>
            <h1 class="font-heading font-semibold mb-2">Inventori</h1>
            <p class="text-muted mb-0">
                Sistem pengurusan inventori kerajaan. Lihat, urus dan cipta rekod inventori dengan mudah.
            </p>
        </div>
        <div class="d-flex gap-2 flex-wrap">
            ...
            @can('viewAny', App\Models\Inventory::class)
                <a href="{{ route('excel.inventory.form') }}"
                   ...
                   aria-label="Import atau export data inventori">
                    Import/Export
                </a>
            @endcan
            @auth
                @if(auth()->user()->hasRole('admin'))
                    <a href="{{ route('inventories.deleted.index') }}"
                       ...
                       aria-label="Lihat inventori yang telah dipadam">
                        Inventori Dipadam
                    </a>
                @endif
            @endauth
...
<section aria-labelledby="filter-heading" class="mb-4">
    <h2 id="filter-heading" class="visually-hidden">Penapis dan carian inventori</h2>
    ...
    <div class="bg-surface p-3 rounded border mb-3">
        ...
        <form method="GET"
              class="myds-grid myds-grid-desktop myds-grid-tablet myds-grid-mobile gap-3"
              aria-label="Penapis carian inventori">

            <div class="mobile:col-span-4 tablet:col-span-4 desktop:col-span-6">
                <label for="search" class="form-label">Cari inventori</label>
                <input id="search"
                       name="search"
                       class="myds-input"
                       value="{{ request('search','') }}"
                       placeholder="Masukkan nama atau keterangan inventori"
                       aria-describedby="search-help" />
                <div id="search-help" class="form-text text-muted">
                    Cari berdasarkan nama atau keterangan item
                </div>
            </div>
            ...
            <div class="mobile:col-span-2 tablet:col-span-2 desktop:col-span-3">
                ...
                <select id="per_page" name="per_page" class="myds-input" onchange="this.form.submit()">
                    @foreach([5,10,15,25,50,100] as $n)
                        <option value="{{ $n }}" {{ (int) request('per_page', 5) === $n ? 'selected' : '' }}>
                            {{ $n }} item
                        </option>
                    @endforeach
                </select>
...
<section aria-labelledby="inventories-heading">
    <h2 id="inventories-heading" class="visually-hidden">Senarai data inventori</h2>

    <div class="bg-surface rounded border">
        <div class="myds-table-responsive">
            <table class="myds-table"
                   aria-describedby="inventory-count"
                   role="table">
                <caption class="visually-hidden">
                    Jadual inventori dengan {{ $inventoryCount }} item. Gunakan butang tindakan untuk lihat detail atau edit item.
                </caption>
                <thead>
                    <tr role="row">
                ...
                <tbody>
                    @forelse ($inventories as $inventory)
                        <tr role="row">
                            <td>
                                <span class="font-mono font-medium">#{{ $inventory->id }}</span>
                            </td>
                            <td>
                                <strong>{{ $inventory->name }}</strong>
                            </td>
                            <td>
                                <span class="badge bg-primary rounded-pill">{{ $inventory->qty }}</span>
                            </td>
                            <td>
                                {{ $inventory->user?->name ?? '—' }}
                            </td>
                            ...
                            <td>
                                <span title="{{ $inventory->description }}">
                            ...
                            <td>
                                <div class="d-flex gap-1">
                                    <a href="{{ route('inventories.show', $inventory->id) }}"
                                       ...
                                       aria-label="Lihat detail {{ $inventory->name }}"
                                       title="Lihat detail">
                                    ...
                                    @can('delete', $inventory)
                                        <form method="POST"
                                            ...
                                            <button type="submit"
                                                    ...
                                                    aria-label="Padam {{ $inventory->name }}"
                                                    title="Padam item">
                                                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
                                                    <polyline points="3,6 5,6 21,6" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                                                    <path d="m19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                                                </svg>
                                                <span class="visually-hidden">Padam</span>
                                            </button>
                                        </form>
                                    @endcan
                                </div>
                        ...
                        <tr role="row">
                            <td colspan="7" class="text-center py-4">
                                <div class="text-muted">
                                    ...
                                    @can('create', App\Models\Inventory::class)
                                        <a href="{{ route('inventories.create') }}" class="myds-btn myds-btn--primary myds-btn--sm">
                                            Cipta Inventori Pertama
                                        </a>
                                    @endcan
                                </div>
                            </td>
