...
<div class="container">
	<div class="row justify-content-center">
		<div class="col-md-8">
			<div class="card">
				<div class="card-header">Ubah Kenderaan</div>
				...
				<div class="card-body">
					...
					<form method="POST" action="{{ route('vehicles.update', $vehicle->id) }}">
						...
						<div class="mb-3">
							...
							<input id="name" name="name" type="text" class="form-control myds-input" value="{{ old('name', $vehicle->name) }}">
						...
						<div class="mb-3">
							...
							@else
								...
								<div class="form-control-plaintext myds-form-plaintext">{{ $vehicle->user?->name ?? '(tiada pemilik)' }}</div>
							@endif
						</div>

						<div class="mb-3">
							<label for="qty" class="form-label">Kuantiti</label>
							<input id="qty" name="qty" type="number" min="0" class="form-control myds-input" value="{{ old('qty', $vehicle->qty ?? 0) }}">
							@error('qty') <div class="text-danger myds-action--danger">{{ $message }}</div> @enderror
						</div>

						<div class="mb-3">
							<label for="price" class="form-label">Harga</label>
							<input id="price" name="price" type="number" step="0.01" min="0" class="form-control myds-input" value="{{ old('price', $vehicle->price ?? '') }}">
							@error('price') <div class="text-danger myds-action--danger">{{ $message }}</div> @enderror
						</div>

						<div class="mb-3">
							<label for="description" class="form-label">Keterangan</label>
							<textarea id="description" name="description" class="form-control myds-textarea">{{ old('description', $vehicle->description) }}</textarea>
